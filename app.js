    ...
    if (includesOD) {
      ...
      if (inputs.covers.consumables) {
        ...
        if (isGCCV) {
          ...
          /* ===== FIX FOR EV PVT CAR (User Request) =====
             - evPvtCar: Consumables on (Basic IDV + Non-Elec + Elec) = Total IDV
             - pvtCar: only Basic IDV (old)
             Change ONLY for Electric Car
          */
          let consumBase = inputs.idv;
          let consumDetailExtra = '';
          if (inputs.vehicleType === 'evPvtCar') {
            consumBase = (inputs.idv || 0) + (inputs.idvNonElec || 0) + (inputs.idvElec || 0);
            consumDetailExtra = ` — EV: Basic ₹${inputs.idv.toLocaleString('en-IN')} + Non-Elec ₹${(inputs.idvNonElec||0).toLocaleString('en-IN')} + Elec ₹${(inputs.idvElec||0).toLocaleString('en-IN')} = Total ₹${consumBase.toLocaleString('en-IN')}`;
          }
          consum = round(consumBase * cRate / 100);
          rows.push({ label: 'Add: Consumables Cover', detail: `${cRate.toFixed(2)} % of ${inputs.vehicleType==='evPvtCar'?'Total IDV':'IDV'} ₹${consumBase.toLocaleString('en-IN')} (age: ${age.toFixed(1)} yr)${consumDetailExtra}`, value: consum, group: 'od' });
          running += consum;
        }
      }
      ...
      if (inputs.covers.batteryProt) {
        ...
        /* ===== FIX FOR EV PVT CAR (User Request) =====
           - evPvtCar: Battery Protection on (Basic + Non-Elec only) — Elec excluded
           - Other: Total IDV (old)
           Change ONLY for Electric Car
        */
        ...
        if (inputs.vehicleType === 'evPvtCar') {
          ...
          batteryDetail = `Basic ₹${inputs.idv.toLocaleString('en-IN')} + Non-Elec ₹${(inputs.idvNonElec||0).toLocaleString('en-IN')} = ₹${batteryBase.toLocaleString('en-IN')} (Elec Accessories excluded for EV)`;
        }
        bp = round(batteryBase * bpRate / 100);
        rows.push({ label: 'Add: Battery Protection', detail: `${bpRate.toFixed(2)} % of ${inputs.vehicleType==='evPvtCar'?'(IDV+Non-Elec)':'IDV'} — ${batteryDetail}${ageNote}`, value: bp, group: 'od' });
        running += bp;
      }
