/* =========================================================
   OICL Premium Calculator V30.0  ·  Main Application
...
(function () {
  ...
  function pickODRate(zonedSlabs, age, ccValue) {
    ...
    /* FIX: Class D (Misc-D / Ambulance) me slab age5 / age7 / age10plus rakhta hai —
       na age7plus hota hai na age10. Pehle 7-10 yrs par teeno miss ho jaate the
       aur rate 0 aa jaata tha (OD premium ₹0). Ab age10plus par fall back karo. */
  ...
  /* ============================================================
     2.  Tabs
     ============================================================ */
  /* Har tab ka apna page heading — Motor wale me hi "Motor Premium
     Calculator" dikhe, Fire me "Fire", Mediclaim me "Medical", PA me "PA". */
  const PAGE_TITLES = {
    calc:    ['Motor Premium Calculator',   'Own Damage (OD) & Third Party (TP) Premium Calculator'],
    fire:    ['Fire Premium Calculator',    'Fire & Allied Perils · Theft / Burglary Premium Calculator'],
    pa:      ['PA Premium Calculator',      'Personal Accident Premium Calculator'],
    med:     ['Medical Premium Calculator', 'Health / Mediclaim Premium Calculator'],
    quote:   ['Quotation',                  'Motor Insurance Quotation'],
    hquote:  ['Health Quotation',           'Health / Mediclaim Insurance Quotation'],
    history: ['Quote History',              'Saved quotations & calculation history'],
    about:   ['About',                      'Oriental Insurance Premium Calculator']
  };

  window.setPageTitle = function (tab) {
    const t = PAGE_TITLES[tab];
    if (!t) return;
    const m = $('pageTitleMain'), sb = $('pageTitleSub');
    if (m)  m.textContent  = t[0];
    if (sb) sb.textContent = t[1];
    document.title = t[0] + ' · Oriental Insurance';
  };

  /* Fire tab ke andar Fire <-> Theft switch par subtitle bhi badle */
  document.addEventListener('click', (e) => {
    const t = e.target.closest && e.target.closest('#tab-fire .op-tab');
    if (!t) return;
    const sb = $('pageTitleSub');
    if (!sb) return;
    sb.textContent = (t.dataset.op === 'theft')
      ? 'Theft / Burglary Premium Calculator'
      : 'Fire & Allied Perils Premium Calculator';
  });
  ...
  $$('.nav-item').forEach((b) => {
    b.addEventListener('click', () => {
      ...
      window.setPageTitle(b.dataset.tab);
    });
  });
  ...
  function updateCCLabel() {
    ...
    if (isPCCVtype) {
      ...
      ccField.value = '0'; /* PCCV: CC not applicable — auto 0 so validation passes */
      ccField.style.opacity = '0.5';
      ccField.style.cursor = 'not-allowed';
      if (ccFieldWrap) ccFieldWrap.style.opacity = '0.55';
      $('ccHint').textContent = 'Auto 0 for PCCV — enter actual seats in Seating Capacity field';
    } else {
      ccField.disabled = false;
      if (ccField.value === '0') ccField.value = ''; /* clear PCCV auto-0 when switching back */
  ...
  function updateAddOnVisibility() {
    ...
    addonIds.forEach(id => {
      ...
      if (allowed) {
        ...
        el.disabled = true;
        el.checked = false;
        el.parentElement.style.display = 'none'; /* hidden instead of greyed (user request) */
      }
    });
    ...
    /* Hide orphan SI boxes when their checkbox got hidden/unchecked above */
    [['persBelong','persBelongSIField'],['passPa','passPaSIField']].forEach(function (pair) {
      const cb = $(pair[0]), fld = $(pair[1]);
      if (cb && fld && (!cb.checked || cb.disabled || (cb.parentElement && cb.parentElement.style.display === 'none'))) {
  ...
  function updatePolicySections() {
    ...
    const vt = $('vehicleType').value;
    /* Policy-Type options: all 4 for Car/2W families (incl. EV); only
       Package + Liability for commercial vehicles. Skipped while restoring
       old quotes so historic data is never corrupted. */
    const ptSel = $('policyType');
    if (ptSel) {
      if (window.__oicRestoring) {
        /* Restoring an old quote: unhide every option so historic data loads intact */
        Array.from(ptSel.options).forEach(function (o) { o.hidden = false; o.disabled = !o.value; });
      } else {
      const showAllPolicies = !vt || ['pvtCar', 'evPvtCar', 'twoWheeler', 'evTwoWheeler'].includes(vt);
      Array.from(ptSel.options).forEach(function (o) {
        if (!o.value) return;
        const keep = showAllPolicies || o.value === 'package' || o.value === 'liability';
        o.hidden = !keep;
        o.disabled = !keep;
      });
      if (policy) {
        const curOpt = ptSel.querySelector('option[value="' + policy + '"]');
        if (curOpt && curOpt.disabled) { ptSel.value = ''; policy = ''; }
      }
      }
    }
  ...
  $('resetBtn').addEventListener('click', () => {
    // Single Vehicle Type dropdown (static options) — just reset the selection
    if ($('vehicleType')) {
      $('vehicleType').value = '';
    }
    $('fuelType').value   = '';
    ...
    $('seating').value    = '';
    $('dor').value        = '';
    $('policyDate').value = todayDDMMYYYY(); /* TEMP hidden: always today */
  ...
  function validateInputs() {
    ...
    checks.forEach(([id, label]) => {
      const el = $(id);
      if (!el || el.disabled) return; /* disabled fields (PCCV auto-0, liability valuation) are not required */
    ...
    if (missing.length || badDates.length) {
      ...
      if (window.proToast) { window.proToast(msg, 'error', 6000); } else { alert(msg); }
      return false;
    }
    ...
    if(dorD && polD && dorD > polD){
      { const m='❌ Date of Registration ('+formatDateDDMMYYYY(dorD)+') cannot be AFTER Policy Start Date ('+formatDateDDMMYYYY(polD)+'). Please correct dates in DD-MM-YYYY format.'; if (window.proToast) window.proToast(m,'error',6000); else alert(m); }
      return false;
    }
  ...
  /* ---- PRO bridge: expose result + events for history/UI modules ---- */
  window.OIC = window.OIC || {};
  window.OIC.getLastResult = function(){ return lastResult; };
  window.OIC.version = '29.0-pro';

  /* ---- Shared PDF helpers (health quotation module inhe reuse karta hai) ---- */
  window.OIC.pdf = {
    renderHTMLToCanvas: function (html) { return renderHTMLToCanvas(html); },
    canvasToA4: function (canvas) { return canvasToSinglePageA4PDF(canvas); },
    download: function (blob, name) { return downloadPDFBlob(blob, name); },
    css: function () { return getQuotePrintCSS(); },
    today: function () { return todayDDMMYYYY(); }
  };
  ...
  function runCalc() {
    const inputs = {
      ...
      isElectric:     (['evPvtCar','evTwoWheeler'].includes($('vehicleType').value) || $('fuelType').value === 'Electric') ? 'Yes' : 'No',
      cc:             num('cc'),
      seating:        num('seating'),
    ...
    if (includesOD) {
      // 1. Basic OD
      /* PCCV: basic rate ab SEATING BAND se resolve hota hai (6-17 / 17-36 / >36),
         taaki dropdown me galat category hone par bhi sahi rate pick ho. */
      const odRateKey = (typeof resolveODRateKey === 'function')
        ? resolveODRateKey(inputs.vehicleType, inputs.seating)
        : inputs.vehicleType;
      const odSlabs = OD_RATES[odRateKey] || OD_RATES[inputs.vehicleType];
      ...
      // 7. Overturning Cover
      //    Class D (Misc-D / Ambulance) -> 5 per mille of IDV.
      //    Baaki vehicles -> 5 % of Basic OD (purana behaviour).
      let overturn = 0;
      if (inputs.covers.overturning) {
        const otPerMille = (ADDON_FACTORS.overturningIdvPerMille || {})[inputs.vehicleType];
        if (otPerMille) {
          const otIdv = Number(inputs.idv) || 0;
          overturn = round(otIdv * otPerMille / 1000);
          rows.push({
            label: 'Add: Overturning Cover',
            detail: otPerMille + ' per mille (\u2030) of IDV \u20b9' + otIdv.toLocaleString('en-IN'),
            value: overturn, group: 'od'
          });
        } else {
      ...
      if (imt23Rate > 0) {
        ...
        /* Class D (Misc-D / Ambulance): IMT-23 Basic + Overturning Cover par lagta hai. */
        const isClassDImt = (inputs.vehicleType === 'miscD' || inputs.vehicleType === 'ambulance');
        const imtBase = isGCCVWithExtras
          ...
          : (isClassDImt ? (basic + overturn) : basic);
        ...
        const imtDetail = isGCCVWithExtras
          ...
          : (isClassDImt && overturn > 0
              ? `${imt23Rate} % of Basic + Overturning (₹${basic.toLocaleString('en-IN')} + ₹${overturn.toLocaleString('en-IN')} = ₹${imtBase.toLocaleString('en-IN')})`
      ...
      if (inputs.covers.consumables) {
        ...
        if (isGCCV) {
          ...
          /* ===== FIX FOR PVT CAR + EV PVT CAR (User Request) =====
             - pvtCar & evPvtCar: Consumables on (Basic IDV + Non-Elec + Elec) = Total IDV
          */
          let consumBase = inputs.idv;
          let consumDetailExtra = '';
          if (['pvtCar','evPvtCar'].includes(inputs.vehicleType)) {
            consumBase = (inputs.idv || 0) + (inputs.idvNonElec || 0) + (inputs.idvElec || 0);
            const tag = inputs.vehicleType==='evPvtCar' ? 'EV' : 'Pvt Car';
            consumDetailExtra = ` — ${tag}: Basic ₹${inputs.idv.toLocaleString('en-IN')} + Non-Elec ₹${(inputs.idvNonElec||0).toLocaleString('en-IN')} + Elec ₹${(inputs.idvElec||0).toLocaleString('en-IN')} = Total ₹${consumBase.toLocaleString('en-IN')}`;
          }
          consum = round(consumBase * cRate / 100);
          rows.push({ label: 'Add: Consumables Cover', detail: `${cRate.toFixed(2)} % of ${['pvtCar','evPvtCar'].includes(inputs.vehicleType)?'Total IDV':'IDV'} ₹${consumBase.toLocaleString('en-IN')} (age: ${age.toFixed(1)} yr)${consumDetailExtra}`, value: consum, group: 'od' });
          running += consum;
        }
      }
      ...
      if (inputs.covers.batteryProt) {
        ...
        /* ===== FIX FOR PVT CAR + EV PVT CAR (User Request) =====
           - pvtCar & evPvtCar: Battery Protection on (Basic + Non-Elec only) — Elec excluded
        */
        let batteryBase = (inputs.idv || 0) + (inputs.idvNonElec || 0) + (inputs.idvElec || 0);
        let batteryDetail = `Total IDV ₹${batteryBase.toLocaleString('en-IN')}`;
        if (['pvtCar','evPvtCar'].includes(inputs.vehicleType)) {
          batteryBase = (inputs.idv || 0) + (inputs.idvNonElec || 0);
          const tag = inputs.vehicleType==='evPvtCar' ? 'EV' : 'Pvt Car';
          batteryDetail = `${tag}: Basic ₹${inputs.idv.toLocaleString('en-IN')} + Non-Elec ₹${(inputs.idvNonElec||0).toLocaleString('en-IN')} = ₹${batteryBase.toLocaleString('en-IN')} (Elec Accessories excluded)`;
        }
        bp = round(batteryBase * bpRate / 100);
        rows.push({ label: 'Add: Battery Protection', detail: `${bpRate.toFixed(2)} % of ${['pvtCar','evPvtCar'].includes(inputs.vehicleType)?'(IDV+Non-Elec)':'IDV'} — ${batteryDetail}${ageNote}`, value: bp, group: 'od' });
        running += bp;
      }
    ...
    // 18. Personal Belongings Cover
      ...
      if (inputs.ncb > 0) {
        ...
        if (isPCCV) {
          ...
          /* NCB FIX: Electric Accessories premium par bhi NCB lena hai (User Request)
             UW Discount ke baad ka Elec Acc premium include karo
          */
          const elAccAfterUW = round(elAcc * (100 - inputs.uwDiscount) / 100);
          ncbBase = Math.max(0, basicAfterUW + ep + elAccAfterUW);
          let ncbParts = [`Basic after UW Disc ₹${basicAfterUW.toLocaleString('en-IN')}`];
          if (ep > 0) ncbParts.push(`EP ₹${ep.toLocaleString('en-IN')}`);
          if (elAccAfterUW > 0) ncbParts.push(`Elec Acc after UW ₹${elAccAfterUW.toLocaleString('en-IN')} (₹${elAcc.toLocaleString('en-IN')} - ${inputs.uwDiscount}% UW)`);
          ncbDetail = `On (${ncbParts.join(' + ')})`;
        }
    ...
    if (includesTP) {
      /* PCCV: TP bhi seating band se resolve (6-17 / 17-36 / >36) */
      const tpRateKey = (typeof resolveODRateKey === 'function')
        ? resolveODRateKey(inputs.vehicleType, inputs.seating)
        : inputs.vehicleType;
      const tpSlabs = TP_RATES[tpRateKey] || TP_RATES[inputs.vehicleType];
      let slabKey = 'annual';
    ...
    /* commission_rates.js agar load na ho to calculation crash na ho —
       live site par asli getCommissionRates() maujood hai, wahi use hogi. */
    const commRates = (typeof getCommissionRates === 'function')
      ? getCommissionRates(inputs.vehicleType, inputs.policyType, age, inputs.cc, inputs.uwDiscount, inputs.state)
      : { od: 0, tp: 0, note: 'commission_rates.js load nahi hua' };
    ...
    /* ---- PRO: notify history/UI modules ---- */
    try {
      window.OIC.lastInputsSnapshot = lastResult.inputs;
      window.dispatchEvent(new CustomEvent('premium:calculated', { detail: lastResult }));
    } catch (e) { console.warn('PRO event dispatch failed', e); }
  }
  ...
  function renderQuotation(r) {
    ...
    /* PCCV: CC field is auto-0 — show Seating Capacity instead so quotation never prints "0 Seats" */
    const isPCCVquote = ['pccvSmall','pccvMedium','pccvLarge','pccvExtraLarge'].includes(i.vehicleType);
    const ccDisplayVal = isPCCVquote ? (i.seating || '\u2014') : i.cc;
    $('qVehicle').textContent = (VEHICLE_META[i.vehicleType]?.label || 'Pvt Car') + ' (' + ccDisplayVal + ' ' + (VEHICLE_META[i.vehicleType]?.unit || 'CC') + ')';
    $('qCc').textContent = ccDisplayVal + ' ' + (VEHICLE_META[i.vehicleType]?.unit || 'CC');
    /* Sum Insured = Vehicle IDV + accessories (\u2014 for Liability-Only / zero IDV) */
    const sumInsuredQ = (Number(i.idv) || 0) + (Number(i.idvNonElec) || 0) + (Number(i.idvElec) || 0);
    if ($('qSumInsured')) $('qSumInsured').textContent = sumInsuredQ > 0 ? fmt(sumInsuredQ) : '\u2014';
    ...
    // For bundled policy: OD = 1 year, TP = 3 years (car) or 5 years (2W)
    let policyEndFormatted, odEndFormatted, tpEndFormatted;
    
    if (i.policyType === 'bundle') {
      // Bundled policy: OD is always 1 year
      odEndFormatted = i.policyDate ? addYearsDDMMYYYY(i.policyDate, 1) : '—';
      // TP is 3 years for car, 5 years for 2W
      tpEndFormatted = i.policyDate ? addYearsDDMMYYYY(i.policyDate, r.years) : '—';
      policyEndFormatted = tpEndFormatted; // Main end date is TP end date
    } else {
      // Package/SAOD/Liability: single term
      policyEndFormatted = i.policyDate ? addYearsDDMMYYYY(i.policyDate, r.years) : '—';
      odEndFormatted = policyEndFormatted;
      tpEndFormatted = policyEndFormatted;
    }
    ...
    // Store for display in period label
    r.odEndFormatted = odEndFormatted;
    r.tpEndFormatted = tpEndFormatted;
    
    // Show OD and TP periods separately for bundled policy
    if (i.policyType === 'bundle') {
      if ($('qOdPeriodWrap')) $('qOdPeriodWrap').style.display = '';
      if ($('qTpPeriodWrap')) $('qTpPeriodWrap').style.display = '';
      if ($('qOdPeriod')) $('qOdPeriod').textContent = `${policyStartFormatted} → ${odEndFormatted} (1 yr)`;
      if ($('qTpPeriod')) $('qTpPeriod').textContent = `${policyStartFormatted} → ${tpEndFormatted} (${r.years} yr)`;
    } else {
      if ($('qOdPeriodWrap')) $('qOdPeriodWrap').style.display = 'none';
      if ($('qTpPeriodWrap')) $('qTpPeriodWrap').style.display = 'none';
    }
    ...
    if($('qPeriodLabel')){
      if (i.policyType === 'bundle') {
        // Bundled policy: show separate OD and TP periods
        $('qPeriodLabel').innerHTML = `OD: ${policyStartFormatted} → ${r.odEndFormatted} (1 yr) | TP: ${policyStartFormatted} → ${r.tpEndFormatted} (${r.years} yr)`;
      } else {
    ...
    /* GST % label — GCCV Liability-Only shows 5% (TP only); otherwise entered rate.
       NOTE: rebuild the label HTML every time — a previous GCCV split render
       replaces this cell's innerHTML (destroying #qGstPct), which used to crash
       all later renders + silently break history auto-save. */
    const qGstRowEl = $('qGstRow');
    if (qGstRowEl) {
      qGstRowEl.querySelector('td:first-child').innerHTML =
        'GST @ <span id="qGstPct">' + ((r.isGCCV && i.policyType === 'liability') ? 5 : i.gst) + '</span>%';
    }
  ...
  /* NOTE: Vehicle Type is a single static dropdown now (all options in index.html).
     The old two-step parent-Type -> Category mapping was removed. */
  ...
  function autoPickUWDiscount() {
    ...
    /* Policy Start Date field is hidden — agar khaali ho to aaj ki date use karo,
       warna auto-pick chup-chaap skip ho jaata tha aur U/W % blank reh jaata tha. */
    let pdV = $('policyDate').value.trim();
    if (!pdV || !isValidDDMMYYYY(pdV)) pdV = todayDDMMYYYY();

    let age = 0, brandNew = false;
    if (dorV && isValidDDMMYYYY(dorV)) {
      age = calcAge(pdV, dorV);
      /* Brand New = DOR same as Policy Start Date */
      brandNew = (formatDateDDMMYYYY(dorV) === formatDateDDMMYYYY(pdV));
    } else {
      return; // DOR abhi incomplete — wait
    }
    ...
    const r = getUWDiscountAuto({
      ...
      policyType:  $('policyType').value,
      brandNew:    brandNew,
      isEV:        (['evPvtCar','evTwoWheeler'].includes(vt) || ($('fuelType') && $('fuelType').value === 'Electric'))
    });
    if (!r) return;
  ...
  /* ------------------------------------------------------------
     PCCV: Seating capacity ke hisaab se category AUTO-SWITCH
     (6-17 / 17-36 / Above 36). Isse basic rate + U/W discount
     hamesha actual seats ke hisaab se pick hota hai.
     ------------------------------------------------------------ */
  function autoSwitchPCCVCategory() {
    const vtEl = $('vehicleType');
    const seatEl = $('seating');
    if (!vtEl || !seatEl || typeof getPCCVSeatBand !== 'function') return;
    const vt = vtEl.value;
    if (!['pccvSmall','pccvMedium','pccvLarge','pccvExtraLarge'].includes(vt)) return;
    const seats = Number(seatEl.value) || 0;
    if (seats <= 0) return;
    const want = getPCCVSeatBand(seats).key;
    if (want !== vt && vtEl.querySelector('option[value="' + want + '"]')) {
      vtEl.value = want;
      const lbl = (VEHICLE_META[want] && VEHICLE_META[want].label) || want;
      if (window.proToast) window.proToast(seats + ' seats → ' + lbl + ' auto-selected', 'info');
      if (typeof updateCCLabel === 'function') updateCCLabel();
      if (typeof updateAddOnVisibility === 'function') updateAddOnVisibility();
    }
  }
  if ($('seating')) {
    ['input','change'].forEach(ev =>
      $('seating').addEventListener(ev, () => { autoSwitchPCCVCategory(); autoPickUWDiscount(); }));
  }

  /* re-pick on relevant field changes */
  ['vehicleType','cc','seating','ncb','idv','policyType','fuelType','dor','policyDate'].forEach(id => {
    const el = $(id);
    if (!el) return;
  ...
  $('quoteBack').addEventListener('click', () => {
    ...
    window.setPageTitle('calc');
  });
  // printQuote bound later to 1-page visual PDF download (not browser print)
  
  $('shareQuote').addEventListener('click', () => {
    if (!lastResult) {
      if (window.proToast) window.proToast('Please calculate premium first before sharing/exporting.','warning'); else alert('Please calculate premium first before sharing/exporting.');
      return;
    }
    const m = $('shareModal');
...
Oriental Insurance`;
  ...
  function buildQuoteHTMLForPDF(r) {
    ...
    const _isPCCVpdf = ['pccvSmall','pccvMedium','pccvLarge','pccvExtraLarge'].includes(i.vehicleType);
    const _ccVal = _isPCCVpdf ? (i.seating || '\u2014') : i.cc;
    const _sumIns = (Number(i.idv) || 0) + (Number(i.idvNonElec) || 0) + (Number(i.idvElec) || 0);
    const veh = (VEHICLE_META[i.vehicleType]?.label || 'Pvt Car') + ' (' + _ccVal + ' ' + (VEHICLE_META[i.vehicleType]?.unit || 'CC') + ')';
    const termLabel = r.years === 5 ? '5-Year' : (r.years === 3 ? '3-Year' : '1-Year');
    ...
    return `<!DOCTYPE html><html><head><meta charset="UTF-8"><style>${getQuotePrintCSS()}</style></head><body>
      <div class="quote-doc" id="pdfRoot">
        ...
        <div class="quote-meta">
          ...
          <div><strong>CC / GVW</strong><span>${esc(_ccVal + ' ' + (VEHICLE_META[i.vehicleType]?.unit || 'CC'))}</span></div>
          <div><strong>Sum Insured (IDV)</strong><span style="color:#047857;font-weight:700;">${esc(_sumIns > 0 ? money(_sumIns) : '\u2014')}</span></div>
          ...
          ${i.policyType === 'bundle' ? `
            <div><strong>OD Period</strong><span style="color:#0369a1;font-weight:700;">${esc(pStart)} → ${esc(r.odEndFormatted || addYearsDDMMYYYY(i.policyDate, 1))} (1 yr)</span></div>
            <div><strong>TP Period</strong><span style="color:#b45309;font-weight:700;">${esc(pStart)} → ${esc(r.tpEndFormatted || pEnd)} (${r.years} yr)</span></div>
          ` : ''}
          <div><strong>State</strong><span>${esc(i.state || '—')}</span></div>
        </div>

        <div class="chip-row">
          ${i.policyType === 'bundle' ? `
            <span class="chip chip-o">OD: ${esc(pStart)} → ${esc(r.odEndFormatted || addYearsDDMMYYYY(i.policyDate, 1))} (1 yr)</span>
            <span class="chip chip-o">TP: ${esc(pStart)} → ${esc(r.tpEndFormatted || pEnd)} (${r.years} yr)</span>
          ` : `
        ...
        <div class="disclaimer">
          ...
          <strong>NCB is applicable only if no claim was reported in the previous policy year.</strong><br>
          <strong class="generated-by">Generated: ${esc(today)} · Customized by Mohit Sharma</strong>
        </div>
      </div>
    </body></html>`;
  }
  ...
  /* =========================================================
     HI-DPI PDF RENDER SETTINGS
     ---------------------------------------------------------
     scale 2 se output blur aa raha tha (~150 DPI). Ab A4 width
     794 CSS px ko target DPI par render karte hain:
        scale 4 -> ~300 DPI  (print quality, "4K")
        scale 6 -> ~450 DPI  (extra sharp, "8K")
     Browser canvas ki hard limit hoti hai, isliye area check
     karke safe scale chunte hain — warna canvas blank aa jata hai.
     ========================================================= */
  var PDF_TARGET_SCALE = 4;         // 794px A4 x4 = 3176px wide ≈ 384 DPI
  var MAX_CANVAS_SIDE  = 16384;     // browser texture limit
  var MAX_CANVAS_PX    = 40000000;  // ~40 MP — isse upar mobile Safari/Chrome
                                    // silently blank canvas de dete hain

  function bestScale(width, height, want) {
    var s = want || PDF_TARGET_SCALE;

    // Mobile / kam RAM wale device par thoda neeche rakho
    var mem = navigator.deviceMemory || 4;
    var mobile = /Android|iPhone|iPad|iPod/i.test(navigator.userAgent || '');
    if (mobile || mem <= 2) s = Math.min(s, 3);

    // Canvas budget me fit karo
    while (s > 1) {
      var w = Math.floor(width * s), h = Math.floor(height * s);
      if (w <= MAX_CANVAS_SIDE && h <= MAX_CANVAS_SIDE && (w * h) <= MAX_CANVAS_PX) break;
      s -= 0.5;
    }
    return Math.max(1, s);
  }
      ...
      setTimeout(() => {
        try {
          ...
          const doSvgSnap = () => {
            ...
            img.onload = () => {
              try {
                const scale = bestScale(width, height);
  ...
  function canvasToSinglePageA4PDF(canvas) {
    ...
    // JPEG quality: 0.93 par text ke around ringing/blur artifacts aate the.
    // 0.985 lossless ke kaafi kareeb hai — file thodi badi, print crisp.
    const dataUrl = canvas.toDataURL('image/jpeg', 0.985);
      ...
      if (liveDoc && typeof window.html2canvas === 'function') {
        ...
        try {
          canvas = await window.html2canvas(liveDoc, {
            scale: bestScale(liveDoc.scrollWidth || 794, liveDoc.scrollHeight || 1123),
            useCORS: true,
            backgroundColor: '#ffffff',
            logging: false,
            imageTimeout: 0
  ...
  async function sharePDFFile(mode) {
    ...
    if (!lastResult) {
      if (window.proToast) window.proToast('Please calculate premium first before sharing.','warning'); else alert('Please calculate premium first before sharing.');
      return;
    }
  ...
  if ($('printQuote')) {
    ...
    $('printQuote').addEventListener('click', () => {
      if (!lastResult) {
        if (window.proToast) window.proToast('Please calculate premium first.','warning'); else alert('Please calculate premium first.');
        return;
      }
  ...
  /* Initial setup (Vehicle Type options are static in HTML now) */

  /* TEMP (user request 07-09-2026): Policy Start Date hidden from the form —
     auto-set to today. To show it again, delete this block + restore the
     reset-handler line to ''. */
  (function hidePolicyStartDate() {
    const pd = $('policyDate');
    if (!pd) return;
    pd.value = todayDDMMYYYY();
    pd.disabled = true;
    pd.classList.add('is-valid');
    const wrap = pd.closest('.field');
  ...
  console.log('%c✓ OICL Premium Calculator V30.0 — DD-MM-YYYY ACTIVE', 'color:#fff;background:#0a2540;padding:6px 12px;border-radius:6px;font-weight:700;');
