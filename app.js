/* =========================================================
   Motor Premium Calculator Pro V 26.9  ·  Main Application
...
(function () {
  ...
  function pickODRate(zonedSlabs, age, ccValue) {
    ...
    if (!slab) return 0;
    if (age <= 5) return slab.age5 || slab.rate;
    if (age <= 7) return slab.age7 || slab.age10 || slab.rate;
    if (age <= 10) return slab.age10 || slab.age7plus || slab.rate;
    return slab.age10plus || slab.age7plus || slab.rate;
  }
  ...
  function runCalc() {
    ...
    const isCommercial = ['taxi','auto','schoolBus','pccvSmall','pccvMedium','pccvLarge','pccvExtraLarge','gccv','gccv3w','miscD','ambulance'].includes(inputs.vehicleType);
    ...
    if (includesOD) {
      ...
      const fixedAdd = FIXED_OD_ADDITION[inputs.vehicleType] || 0;
      ...
      if (fixedAdd > 0) {
        ...
        rows.push({ label: 'Add: Fixed OD Addition', detail: `₹ ${fixedAdd} (PCCV ${VEHICLE_META[inputs.vehicleType]?.label || ''})`, value: fixedAdd, group: 'od' });
      } else {
      ...
      if (inputs.covers.consumables) {
        ...
        const isOtherCommercial = ['auto','schoolBus','pccvSmall','pccvMedium','pccvLarge','pccvExtraLarge','miscD','ambulance'].includes(inputs.vehicleType);
    ...
    // 18. Personal Belongings Cover
      ...
      if (inputs.ncb > 0) {
        ...
        if (isPCCV) {
          ...
          ncbBase = Math.max(0, postUWBase);
          ncbDetail = `On OD Premium after IMT-23 & U/W Disc ₹${postUWBase.toLocaleString('en-IN')}`;
    ...
    if (includesTP) {
      ...
      const paxCount = isPCCVtp ? inputs.seating : Math.max(0, inputs.seating - 1);
      let ppTP = 0;
      ...
      if (inputs.llCount > 0) {
        const isLLCommercial = ['taxi','auto','schoolBus','pccvSmall','pccvMedium','pccvLarge','pccvExtraLarge'].includes(inputs.vehicleType);
      ...
      if (inputs.covers.passPa && inputs.seating > 1) {
        ...
        const siPerPerson = TP_FLAT.passPA.defaultSI || 100000;
        ...
        tpRows.push({
          ...
          value: passPA,
          group: 'tp',
          detail: `SI ${fmt(siPerPerson)} per person`
        });
        tpRun += passPA;
      }
    ...
    /* ---- Commission (Apr-Jun 2026 structure):
           OD premium × odRate%  +  TP premium × tpRate% ---- */
    const commRates = getCommissionRates(inputs.vehicleType, inputs.policyType, age, inputs.cc);
  ...
  function renderResults(r) {
    ...
    if ($('commissionDetail')) $('commissionDetail').textContent = '(OD ' + r.commissionOdRate + '% + TP ' + r.commissionTpRate + '%)';
    ...
    /* GCCV split GST breakdown display removed per request — falls through to generic GST total */
    if (false) {
...
Oriental Insurance`;
      ...
      <div class="quote-doc" id="pdfRoot">
        ...
        <table class="ft">
          <tbody>
            ...
            <tr><td>GST @ ${esc(gstPct)}%</td><td class="right">${esc(money(r.gstAmt))}</td></tr>
  ...
  console.log('%c✓ Motor Calculator V26.9 — DD-MM-YYYY ACTIVE', 'color:#fff;background:#0a2540;padding:6px 12px;border-radius:6px;font-weight:700;');
