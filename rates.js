/* =========================================================
   OICL Premium Calculator V30.0  ·  Rate Tables
   Based on:
...
/* ============================================================
   10Z.  PCCV SEATING BANDS  (added 08-Sep-2026)
        Single source of truth — seating capacity se hi band,
        basic OD rate aur U/W discount decide hota hai.
        Bands:  6–17  ·  17–36  ·  Above 36
   ============================================================ */
var PCCV_SEAT_BANDS = [
  { key: 'pccvSmall',  label: '6-17 seats',    minSeats: 0,  maxSeats: 17 },
  { key: 'pccvMedium', label: '17-36 seats',   minSeats: 18, maxSeats: 36 },
  { key: 'pccvLarge',  label: 'Above 36 seats',minSeats: 37, maxSeats: Infinity }
];

/* Seating capacity -> band. Old 'pccvExtraLarge' quotes map to pccvLarge. */
function getPCCVSeatBand(seats) {
  seats = parseFloat(seats) || 0;
  for (var i = 0; i < PCCV_SEAT_BANDS.length; i++) {
    if (seats <= PCCV_SEAT_BANDS[i].maxSeats) return PCCV_SEAT_BANDS[i];
  }
  return PCCV_SEAT_BANDS[PCCV_SEAT_BANDS.length - 1];
}

/* U/W discount table per seating band.
   newVeh = new vehicle / 1st year · u10 = upto 10 yr · a10 = above 10 yr
   N = with NCB (>=20%) · X = without NCB */
var PCCV_UW_DISCOUNT = {
  pccvSmall:  { newVeh: 75, u10N: 85, u10X: 70, a10N: 80, a10X: 50 },
  pccvMedium: { newVeh: 75, u10N: 80, u10X: 65, a10N: 70, a10X: 40 },
  pccvLarge:  { newVeh: 75, u10N: 75, u10X: 60, a10N: 60, a10X: 40 }
};

/* Basic OD rate lookup key — PCCV ke liye seating band se resolve hota hai
   taaki galat category select hone par bhi sahi rate pick ho. */
function resolveODRateKey(vehicleType, seats) {
  if (['pccvSmall','pccvMedium','pccvLarge','pccvExtraLarge'].indexOf(vehicleType) !== -1) {
    return getPCCVSeatBand(seats).key;
  }
  return vehicleType;
}
...
/* ============================================================
   13. NIL DEP (ND) ADD-ON COVER RATES — % of IDV
       ...
       Two Wheeler -> CC-based (Upto 150 CC / Above 150 CC).
                   CORRECTION 24-Sep-2026: anything ABOVE 150 CC (151-350
                   AND >350) now uses the "350 CC" rate table: 0.40 / 0.50 /
                   0.70 / 0.80 / 1.20 / 1.30 / 1.50 (% of IDV).
   ...
   ============================================================ */
/* ---------- TWO WHEELER · Dep Cap / Nil Dep rate table (ABOVE 150 CC) ----------
   CORRECTION (24-Sep-2026):
     Two wheelers ABOVE 150 CC — i.e. the 151-350 CC bucket AND the
     above-350 CC bucket — now use ONE common table: the "350 CC" rates.
       0.40 | 0.50 | 0.70 | 0.80 | 1.20 | 1.30 | 1.50  (% of IDV)
     (Only the 0-150 CC bucket keeps 0.25 | 0.40 | 0.60 | ...)
   Both buckets point to this SAME array (by reference), so they can never
   drift apart again. Applies to Petrol 2W and EV 2W alike. */
var TWO_WHEELER_ND_ABOVE_150 = [
  { max: 1, rate: 0.40 },     // 0-1 Year
  { max: 2, rate: 0.50 },     // 1-2 Year
  { max: 3, rate: 0.70 },     // 2-3 Year
  { max: 4, rate: 0.80 },     // 3-4 Year
  { max: 5, rate: 1.20 },     // 4-5 Year
...
var NIL_DEP_RATE = {
  ...
  /* ---------- TWO WHEELER — CC-based, % of IDV ----------
     Two effective buckets after the 24-Sep-2026 correction:
       A: upto 150 CC  -> ccUpto150 table (0.25 / 0.40 / 0.60 / ...)
       B: ABOVE 150 CC -> cc150To350 AND above350 BOTH use the shared
                          TWO_WHEELER_ND_ABOVE_150 table (0.40 / 0.50 / ...)
  ...
  twoWheeler: {
    ...
    cc150To350: TWO_WHEELER_ND_ABOVE_150,  /* B — above 150 CC upto 350 CC (now = 350 CC table) */
    above350:   TWO_WHEELER_ND_ABOVE_150   /* C — above 350 CC (same table, shared by reference) */
  },
...
schoolBus: 15, staffBus: 15,
  ...
  /* Overturning Cover.
     Class D (Misc-D / Ambulance): 5 per mille (0.5%) of IDV.
     Baaki vehicles: 5% of Basic OD (purana behaviour). */
  overturning: { flat: 0 },
  overturningIdvPerMille: {
    miscD:     5,
    ambulance: 5
  },
...
/* ============================================================
   21. COMMISSION — Now in separate file: commission_rates.js
       ...
       for any commission-related changes.
   ============================================================ */
...
/* ============================================================
   ...
   Eligibility rules auto-enforced (returns 0 if NOT eligible):
     ...
     • Two Wheeler / EV 2W: age > 7 yrs -> 0 ; age > 5 yrs and NCB < 20% -> 0
       CC buckets: upto 150 | ABOVE 150 (>150-350 AND >350 share the same
       table). EV 2W: >16 KW uses the above-150 column, else upto-150.
   ...
   Usage:
     ...
     getNilDepRate('twoWheeler', 'petrol',  0.5, 160)       => 0.40  (above 150 CC -> 350 CC table)
     getNilDepRate('twoWheeler', 'petrol',  3, 160)         => 0.70  (above 150 CC)
     getNilDepRate('twoWheeler', 'petrol',  3, 400)         => 0.70  (>350 CC, same table)
...
function getNilDepRate(vehicleType, fuelType, ageInYears, ccOrKw, ncbPct) {
  ...
  if (vehicleType === 'twoWheeler') {
    ...
    /* Decide CC / KW bucket: A = upto 150, B = above 150 (B and C share the
       same table, so >350 CC and 151-350 CC now return identical rates).
       EV 2W (KW) parity with the OD slabs: > 16 KW behaves like a >350cc
       performance machine and takes the above-150 column; A column otherwise. */
...
function checkNilDepEligibility(vehicleType, fuelType, ageInYears, ccOrKw, ncbPct) {
  ...
  /* --- Two Wheeler: Dep Cap / ND available upto 7 yrs ---
...
var VEHICLE_META = {
  ...
  pccvMedium: { label: 'PCCV (17–36 seats)',    unit: 'Seats', ccHint: 'Seating Capacity', allowFiveYear: false },
  pccvLarge:  { label: 'PCCV (Above 36 seats)',    unit: 'Seats', ccHint: 'Seating Capacity', allowFiveYear: false },
  pccvExtraLarge:  { label: 'PCCV (Above 36 seats)',  unit: 'Seats', ccHint: 'Seating Capacity', allowFiveYear: false },
...
function getUWDiscountAuto(o) {
  ...
  /* ---------------- TAXI (PCCV upto 6 seats) ----------------
     Corrected 08-Sep-2026: NEW taxi (1st year) = 90% allowed.
     Pehle brand-new par galti se 85% cap lag raha tha. */
  if (vt === 'taxi') {
    if (brandNew || age < 1) return res(90, 'Taxi New Vehicle / 1st Year — max 90%');
    if (age <= 10) return res(withNCB ? 85 : 75, 'Taxi Upto 10 yr ' + ncbTxt);
  ...
  /* NEW-VEHICLE RULE (corrected 08-Sep-2026):
       School Bus / Staff Bus  -> 97%
       Normal PCCV buses       -> 75%   (pehle galti se 97% pick ho raha tha)
     "New" = brand new (DOR = policy start) OR age < 1 year. */
  const isSchoolStaff = (vt === 'schoolBus' || vt === 'staffBus');
  const isPCCVbus = ['pccvSmall','pccvMedium','pccvLarge','pccvExtraLarge'].includes(vt);
  const isNew = brandNew || age < 1;

  if (isPCCVbus) {
    /* SEATING-DRIVEN BAND (corrected 08-Sep-2026)
       Discount ab actual seating capacity se uthta hai, dropdown label se nahi.
       Bands: 6-17 · 17-36 · Above 36                                        */
    var band = getPCCVSeatBand(seats || 0);
    var B = PCCV_UW_DISCOUNT[band.key];
    if (isNew) return res(B.newVeh, 'PCCV ' + band.label + ' — New Vehicle (max ' + B.newVeh + '%)');
    if (age <= 10) return res(withNCB ? B.u10N : B.u10X, 'PCCV ' + band.label + ' Upto 10 yr ' + ncbTxt);
    return res(withNCB ? B.a10N : B.a10X, 'PCCV ' + band.label + ' Above 10 yr ' + ncbTxt);
  }

  if (isSchoolStaff && isNew) {
    const _lbl = (typeof VEHICLE_META !== 'undefined' && VEHICLE_META[vt]) ? VEHICLE_META[vt].label : 'Bus';
    return res(97, _lbl + ' New Vehicle — max 97%');
  }
