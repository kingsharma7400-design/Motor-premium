...
/* ============================================================
   21. COMMISSION — Oriental Insurance
       MOTOR COMMISSION STRUCTURE : APR-2026 TO JUN-2026
       Commission is paid SEPARATELY on OD and TP premiums:
         commission = round(OD_premium × odRate%)
                    + round(TP_premium × tpRate%)
       Rates are taken directly from the official structure PDF.
       GLOBAL RULE (PDF footnote): "No Commission on Package
       Policies of age > 15 years".
   ============================================================ */
/* ============================================================
   21. COMMISSION — Oriental Insurance
       MOTOR COMMISSION STRUCTURE : w.e.f. 01.07.2026
       Source: CIRCULAR-8811
       Commission is paid SEPARATELY on OD and TP premiums:
         commission = round(OD_premium × odRate%)
                    + round(TP_premium × tpRate%)
       
       DISCOUNT TIERS (based on U/W Discount % given to customer):
         • Tier 1: Upto 40% discount
         • Tier 2: 40-70% discount
         • Tier 3: >70% discount
       
       GLOBAL RULE: "No Commission on Package
       Policies of age > 15 years".
   ============================================================ */
...
function gccvComm(gvw, policyType, uwDiscount) {
  gvw = parseFloat(gvw) || 0;
  var tier = getCommTier(uwDiscount);
  
  // New vehicle GCCV: discount-tiered commissions
  // Tier 1 (≤40% disc), Tier 2 (40-70%), Tier 3 (>70%)
  var newRows = [
    { max: 2000,     t1:{lO:55,lT:55,pO:55,pT:55}, t2:{lO:55,lT:55,pO:55,pT:55}, t3:{lO:55,lT:55,pO:55,pT:55} },
    { max: 3500,     t1:{lO:40,lT:40,pO:45,pT:45}, t2:{lO:45,lT:45,pO:50,pT:50}, t3:{lO:50,lT:50,pO:55,pT:55} },
    { max: 7500,     t1:{lO:25,lT:25,pO:32.5,pT:32.5}, t2:{lO:30,lT:30,pO:37.5,pT:37.5}, t3:{lO:35,lT:35,pO:40,pT:40} },
    { max: 10000,    t1:{lO:15,lT:15,pO:15,pT:15}, t2:{lO:20,lT:20,pO:20,pT:20}, t3:{lO:25,lT:25,pO:25,pT:25} },
    { max: 12500,    t1:{lO:5,lT:5,pO:5,pT:5}, t2:{lO:7.5,lT:7.5,pO:10,pT:10}, t3:{lO:10,lT:10,pO:12.5,pT:12.5} },
    { max: 20000,    t1:{lO:7.5,lT:7.5,pO:7.5,pT:7.5}, t2:{lO:10,lT:10,pO:12.5,pT:12.5}, t3:{lO:12.5,lT:12.5,pO:15,pT:15} },
    { max: 25000,    t1:{lO:10,lT:10,pO:10,pT:10}, t2:{lO:12.5,lT:12.5,pO:15,pT:15}, t3:{lO:15,lT:15,pO:17.5,pT:17.5} },
    { max: 34000,    t1:{lO:7.5,lT:7.5,pO:7.5,pT:5}, t2:{lO:10,lT:10,pO:10,pT:7.5}, t3:{lO:12.5,lT:12.5,pO:12.5,pT:10} },
    { max: 40000,    t1:{lO:0,lT:0,pO:7.5,pT:5}, t2:{lO:5,lT:5,pO:10,pT:7.5}, t3:{lO:7.5,lT:7.5,pO:10,pT:10} },
    { max: 50000,    t1:{lO:0,lT:0,pO:5,pT:2.5}, t2:{lO:2.5,lT:2.5,pO:7.5,pT:5}, t3:{lO:5,lT:5,pO:10,pT:7.5} },
    { max: Infinity, t1:{lO:0,lT:0,pO:2.5,pT:0}, t2:{lO:2.5,lT:2.5,pO:5,pT:2.5}, t3:{lO:5,lT:5,pO:7.5,pT:5} }
  ];
  
  var r = newRows[newRows.length - 1];
  for (var i = 0; i < newRows.length; i++) {
    if (gvw <= newRows[i].max) { r = newRows[i]; break; }
  }
  
  var tierData = (tier === 1) ? r.t1 : (tier === 2) ? r.t2 : r.t3;
  
  if (policyType === 'liability') return { od: tierData.lO, tp: tierData.lT };
  return { od: tierData.pO, tp: tierData.pT };
}
...
/* Commission profiles — w.e.f. 01.07.2026 (CIRCULAR-8811)
   A profile entry may be:
     ...
     • a function(age, gvw, uwDiscount) returning { od, tp }
   od = OD commission %, tp = TP commission %. */
var COMMISSION_PROFILES = {
  
  /* ---------- PRIVATE CAR (w.e.f. 01.07.2026) ----------
     New Vehicle: Discount-tiered on Bundled Policy
       Tier 1 (Upto 40% disc): OD 20%, TP 50%
       Tier 2 (40-70% disc):   OD 25%, TP 60%
       Tier 3 (>70% disc):     OD 30%, TP 70%
     Old Vehicle (existing): 20% on net premium (package)
     SAOD: 20% on OD
     Liability: 20% on TP */
  pvtCar: {
    liability: { od: 0, tp: 20 },
    saod:      { od: 20, tp: 0 },
    bundle: function(age, gvw, uwDiscount) {
      var tier = getCommTier(uwDiscount);
      if (age < 1) { // New vehicle
        if (tier === 1) return { od: 20, tp: 50 };
        if (tier === 2) return { od: 25, tp: 60 };
        return { od: 30, tp: 70 }; // tier 3
      }
      // Old vehicle - existing
      return { od: 20, tp: 20 };
    },
    package: [
      { maxAge: 15,       od: 20, tp: 20 },
      { maxAge: Infinity, od: 0,  tp: 20 }
    ]
  },
  
  /* ---------- TWO WHEELER (w.e.f. 01.07.2026) ----------
     New Scooter (any CC, <1yr): 40% OD + 90% TP (1st year only)
     New Bike by CC range & discount tier:
       ≤150cc: Tier1(40/90), Tier2(45/95), Tier3(50/100)
       150-350cc: Tier1(35/85), Tier2(40/90), Tier3(45/95)
       >350cc: Tier1(30/80), Tier2(35/85), Tier3(40/90)
     Old Two Wheeler:
       ≤150cc: upto 3yr→50%, 3-7yr→30%, >7yr→20%
       >150cc: upto 3yr→50%, 3-7yr→30%, >7yr→20%
     SAOD: 20%
     Liability: 20% on TP */
  twoWheeler: {
    ...
    bundle: function(age, gvw, uwDiscount) {
      ...
      if (age < 1) {
        // New vehicle - scooters vs bikes
        // Scooter: any CC, any tier → 40% OD + 90% TP
        if (tier === 1) {
          if (cc <= 150) return { od: 40, tp: 90 };
          if (cc <= 350) return { od: 35, tp: 85 };
          return { od: 30, tp: 80 };
        }
        if (tier === 2) {
          if (cc <= 150) return { od: 45, tp: 95 };
          if (cc <= 350) return { od: 40, tp: 90 };
          return { od: 35, tp: 85 };
        }
        // tier 3
        if (cc <= 150) return { od: 50, tp: 100 };
        if (cc <= 350) return { od: 45, tp: 95 };
        return { od: 40, tp: 90 };
      }
      
      // Old vehicle commission by age
      if (age <= 3) return { od: 50, tp: 20 };
      if (age <= 7) return { od: 30, tp: 20 };
      return { od: 20, tp: 20 };
    },
    package: [
      { maxAge: 15,       od: 20, tp: 20 },
  ...
  /* ---------- TAXI (4W Passenger < 6 seats) (w.e.f. 01.07.2026) ----------
     New: 20% OD + 15% TP (Bundle)
     Old: 15% OD + 15% TP (Package)
     SAOD: 20% on OD
     Liability: 20% on TP */
  taxi: {
    liability: { od: 0, tp: 20 },
    saod:      { od: 20, tp: 0 },
    bundle: function(age, gvw, uwDiscount) {
      if (age < 1) return { od: 20, tp: 15 }; // New
      return { od: 15, tp: 15 };              // Old
    },
    package: [
      { maxAge: 1,        od: 20, tp: 15 },
      { maxAge: Infinity, od: 15, tp: 15 }
    ]
  },
  
  /* ---------- SCHOOL BUS (w.e.f. 01.07.2026) ----------
     Package: 65% on net premium
  ...
  /* ---------- PCCV (Passenger Carrying Vehicles) (w.e.f. 01.07.2026) ----------
     ...
     4 Wheeler PCCV (6-17 seats): New 15%, Old 10%
     4 Wheeler PCCV (17-36 seats): OD 7.5% + TP 2.5%
     4 Wheeler PCCV (>36 seats): 5% (net premium)
     2 Wheeler PCCV: 10% on net premium */
  pccv: {
    liability: function(age, gvw, uwDiscount) {
      var seats = parseFloat(gvw) || 0;
      var tier = getCommTier(uwDiscount);
      if (seats <= 6) {
        // 3 Wheeler / Taxi
        return age < 1 ? { od: 0, tp: 45 } : { od: 0, tp: 35 };
      }
      if (seats <= 17) {
        return age < 1 ? { od: 0, tp: 25 } : { od: 0, tp: 15 };
      }
      return { od: 0, tp: 5 };
    },
    saod: function(age, gvw, uwDiscount) {
      var seats = parseFloat(gvw) || 0;
    ...
    bundle: function(age, gvw, uwDiscount) {
      ...
      if (seats <= 6) {
        // 3 Wheeler / Taxi
        return age < 1 ? { od: 45, tp: 45 } : { od: 35, tp: 35 };
      }
      if (seats <= 17) {
        return age < 1 ? { od: 15, tp: 15 } : { od: 10, tp: 10 };
      }
      if (seats <= 36) {
        return { od: 7.5, tp: 2.5 };
      }
      return { od: 5, tp: 5 };
    },
    package: function(age, gvw, uwDiscount) {
      var seats = parseFloat(gvw) || 0;
      if (seats <= 6) {
        return age < 1 ? { od: 45, tp: 45 } : { od: 35, tp: 35 };
      }
  ...
  /* ---------- GCCV / GOODS VEHICLE (w.e.f. 01.07.2026) ----------
     Commission tiered by U/W discount (Tier 1: ≤40%, Tier 2: 40-70%, Tier 3: >70%)
     GVW ranges with progressive rates */
  gccv: {
    liability: function(age, gvw, uwDiscount) { return gccvComm(gvw, 'liability', uwDiscount); },
    saod:      function(age, gvw, uwDiscount) {
      var tier = getCommTier(uwDiscount);
    ...
    package:   function(age, gvw, uwDiscount) { return gccvComm(gvw, 'package', uwDiscount); },
    bundle:   function(age, gvw, uwDiscount) { return gccvComm(gvw, 'package', uwDiscount); }
  },
  
  /* ---------- MISC. D (w.e.f. 01.07.2026) ----------
     Ambulances: 20% on net premium / 20% on TP
     Agricultural Tractors: 55% (or 2.5% above 80% discount)
     Other Misc. Vehicles: 12.5% / 10% / 8% depending on age */
  miscD: {
    liability: { od: 0, tp: 20 },   // Ambulance: 20% on TP
    saod:      function(age, gvw, uwDiscount) {
      var tier = getCommTier(uwDiscount);
      // Tractors above 80% discount: 2.5%
      if (uwDiscount > 80) return { od: 2.5, tp: 0 };
      return { od: 12.5, tp: 0 };
    },
    package: [
      { maxAge: 5,        od: 12.5, tp: 20 },  // Ambulance/Misc < 5yr
      { maxAge: 10,       od: 12.5, tp: 8 },   // Ambulance 5-10yr / Tractors
      { maxAge: 15,       od: 10,   tp: 8 },   // 10-15yr
      { maxAge: Infinity, od: 10,   tp: 8 }    // >15yr
    ],
    bundle: [
      { maxAge: 5,        od: 12.5, tp: 20 },
      { maxAge: 10,       od: 12.5, tp: 8 },
      { maxAge: 15,       od: 10,   tp: 8 },
      { maxAge: Infinity, od: 10,   tp: 8 }
    ]
  },
  ...
  auto: {
    ...
    saod: function(age, gvw, uwDiscount) {
      var seats = parseFloat(gvw) || 0;
      if (seats <= 6) return { od: 10, tp: 0 };  // 3 Wheeler
      return { od: 10, tp: 0 };                   // 2 Wheeler PCCV
    },
    package: function(age, gvw, uwDiscount) {
      var seats = parseFloat(gvw) || 0;
      return seats <= 6
...
function getCommissionRates(vehicleType, policyType, age, gvw, uwDiscount) {
  ...
  function resolveEntry(entry) {
    ...
    if (typeof entry === 'function') return entry(age, gvw, uwDiscount) || { od: 0, tp: 0 };
  ...
  if (rates) return { od: rates.od, tp: rates.tp, note: '' };
