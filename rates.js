...
var COMMISSION_PROFILES = {
  ...
  staffBus: {                   // PDF: Staff Bus (mirrors School Bus)
    liability: { od: 0, tp: 45 },
    package:   { od: 60, tp: 60 }
  },
...
function getUWDiscountAuto(o) {
  ...
  /* ---------------- PCCV / BUSES (brandNew handled like other vehicles) ---------------- */
  const isBus = ['pccvSmall','pccvMedium','pccvLarge','pccvExtraLarge','schoolBus','staffBus'].includes(vt);
  if (isBus && brandNew) return res(withNCB ? 97 : 85, (VEHICLE_META[vt] ? VEHICLE_META[vt].label : 'Bus') + ' Brand New ' + ncbTxt);
