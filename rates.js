/* =========================================================
   Motor Premium Calculator Pro V 26.7  ·  Rate Tables
   Based on:
...
var OD_RATES = {
  ...
  miscD: {
    zoneA: [
      { max: Infinity, age5: 1.208, age10: 1.238, age10plus: 1.268 }
    ],
    zoneB: [
      { max: Infinity, age5: 1.202, age10: 1.232, age10plus: 1.232 }
    ],
    zoneC: [
      { max: Infinity, age5: 1.190, age10: 1.220, age10plus: 1.250 }
    ]
  },
  ambulance: {
    zoneA: [
...
var ADDON_FACTORS = {
  ...
  rti: {
    ...
    taxi: 0.30, auto: 0.30, schoolBus: 0.30,
  ...
  tyreRim: {
    ...
    taxi: 0.30, auto: 0.20, schoolBus: 0.20,
  ...
  altCar: {
    ...
    taxi: 0.10, auto: 0.10,
    schoolBus: 0, pccvSmall: 0, pccvMedium: 0, pccvLarge: 0, pccvExtraLarge: 0,
  ...
  batteryProt: {
    ...
    taxi: 0.10, auto: 0.10,
    schoolBus: 0.10, pccvSmall: 0.10, pccvMedium: 0.10,
  ...
  elecAcc: {
    ...
    taxi: 4.00, auto: 4.00, schoolBus: 4.00,
  ...
  nonElecAcc: {
    ...
    taxi: 2.00, auto: 2.00, schoolBus: 2.00,
  ...
  persBelong: {
    ...
    taxi: 0.05, auto: 0.05, schoolBus: 0.05,
...
/* ============================================================
   ...
   ============================================================ */
var COMMISSION_PCT = 15; // legacy flat default (kept for reference only)
...
function gccvComm(gvw, policyType) {
  gvw = parseFloat(gvw) || 0;
  var rows = [
    { max: 2000,     lO:55,   lT:55,   pO:55,   pT:55 },
    { max: 3500,     lO:40,   lT:40,   pO:45,   pT:45 },
    { max: 7500,     lO:25,   lT:25,   pO:32.5, pT:32.5 },
    { max: 10000,    lO:15,   lT:15,   pO:15,   pT:15 },
    { max: 12500,    lO:5,    lT:5,    pO:5,    pT:5 },
    { max: 20000,    lO:7.5,  lT:7.5,  pO:7.5,  pT:7.5 },
    { max: 25000,    lO:10,   lT:10,   pO:10,   pT:10 },
    { max: 34000,    lO:7.5,  lT:7.5,  pO:7.5,  pT:5 },
    { max: 40000,    lO:0,    lT:0,    pO:7.5,  pT:5 },
    { max: 50000,    lO:0,    lT:0,    pO:5,    pT:2.5 },
    { max: Infinity, lO:0,    lT:0,    pO:2.5,  pT:0 }
  ];
  var r = rows[rows.length - 1];
  for (var i = 0; i < rows.length; i++) {
    if (gvw <= rows[i].max) { r = rows[i]; break; }
  }
  if (policyType === 'liability') return { od: r.lO, tp: r.lT };
  return { od: r.pO, tp: r.pT };
}
...
var COMMISSION_PROFILES = {
  ...
  taxi: {                       // PDF: PCCV – Taxi
    liability: { od: 0, tp: 5 },
    package: [
      { maxAge: 1,        od: 20, tp: 20 },
      { maxAge: Infinity, od: 15, tp: 15 }
    ]
  },
  schoolBus: {                  // PDF: School/Staff Bus
    liability: { od: 0, tp: 45 },
    package:   { od: 60, tp: 60 }
  },
  pccv: {                      // PDF: generic PCCV (seat-based buses)
    liability: function(age){ return age <= 17 ? { od:0, tp:5 } : { od:0, tp:0 }; },
    package: function(age){
      if (age <= 17) return { od:15, tp:15 };
      if (age <= 36) return (age <= 10) ? { od:7.5, tp:2.5 } : { od:5, tp:2.5 };
      return { od:5, tp:5 };
    }
  },
  gccv: {
    liability: function(age, gvw){ return gccvComm(gvw, 'liability'); },
    package:   function(age, gvw){ return gccvComm(gvw, 'package'); }
  },
  miscD: {                     // PDF: Misc Class D
    liability: { od: 0,  tp: 8 },
    package: [
  ...
  ambulance: {                 // PDF: Ambulance
    liability: { od: 0,  tp: 10 },
    package:   { od: 20, tp: 20 }
  },
  auto: {                      // PDF: no separate row → treated like generic PCCV
    liability: { od: 0, tp: 5 },
    package:   { od: 15, tp: 15 }
  }
};
...
function getCommissionRates(vehicleType, policyType, age, gvw) {
  ...
  function resolveEntry(entry) {
    ...
    if (typeof entry === 'function') return entry(age, gvw) || { od: 0, tp: 0 };
